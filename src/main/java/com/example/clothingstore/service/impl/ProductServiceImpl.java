...
public class ProductServiceImpl implements ProductService {
  ...
  public ProductResDTO createProduct(ProductReqDTO productReqDTO) {
    List<Product> existingProducts =
        productRepository.findByNameAndIsDeletedFalse(productReqDTO.getName());
    ...
    product.setVariants(productReqDTO.getVariants().stream().map(variant -> {
      ...
      productVariant.setQuantity(variant.getQuantity() != null ? variant.getQuantity() : 0);
      ...
      List<ProductImage> variantImages =
          IntStream.range(0, variant.getImages().size()).mapToObj(index -> {
            ProductImage productImage = new ProductImage();
  ...
  // .filter(product -> {
  // Double productMinPriceWithDiscount =
  // promotionCalculatorService.calculateMinPriceWithDiscount(product);
  ...
  // comparator = comparator == null ? currentComparator :
  ...
  public ResultPaginationDTO getProducts(Boolean isBestSeller, Boolean isDiscounted, Integer days,
    ...
    if (averageRating != null) {
      finalSpec =
          finalSpec.and(ProductSpecification.averageRatingGreaterThanOrEqualTo(averageRating));
    }
  ...
  private void updateProductImages(Product product, List<String> newImageUrls) {
    ...
    for (int i = 0; i < newImageUrls.size(); i++) {
      ...
      ProductImage productImage =
          currentImages.stream().filter(img -> img.getPublicUrl().equals(imageUrl)).findFirst()
  ...
  private void updateProductVariants(Product product,
      List<ProductReqDTO.ProductVariantReqDTO> variantReqDTOs) {
    ...
    for (ProductReqDTO.ProductVariantReqDTO variantDTO : variantReqDTOs) {
      ...
      if (variantDTO.getId() != null) {
        // Existing variant
        productVariant = currentVariants.stream().filter(v -> v.getId().equals(variantDTO.getId()))
  ...
  private void updateVariantImages(ProductVariant variant, List<String> imageUrls) {
    ...
    for (int i = 0; i < imageUrls.size(); i++) {
      ...
      ProductImage productImage = variantImages.stream()
          .filter(img -> img.getPublicUrl().equals(imageUrl)).findFirst().orElseGet(() -> {
  ...
  public ProductResDTO convertToProductResDTO(Product product) {
    return ProductResDTO.builder().id(product.getId()).name(product.getName())
        ...
        .averageRating(
            calculateAverageRating(product) == 0 ? null : calculateAverageRating(product))
        .numberOfReviews(calculateNumberOfReviews(product))
        .numberOfSold(calculateNumberOfSold(product)).slug(product.getSlug())
        .colorDefault(product.getColorDefault() != null ? product.getColorDefault().name() : null)
        .images(product.getImages().stream().map(ProductImage::getPublicUrl)
            .collect(Collectors.toList()))
        .variants(product.getVariants().stream()
            .map(variant -> ProductResDTO.ProductVariantResDTO.builder().id(variant.getId())
                ...
                .differencePrice(variant.getDifferencePrice()).images(variant.getImages().stream()
                    .map(ProductImage::getPublicUrl).collect(Collectors.toList()))
                .build())
            .collect(Collectors.toList()))
        .build();
  }
  ...
  private Integer calculateVariantQuantity(ProductVariant variant) {
    ...
    List<CartItem> cartItems =
        cartRepository.findByUserEmail(email).map(Cart::getCartItems).orElse(null);
    ...
    return cartItems.stream()
        ...
        .map(CartItem::getQuantity).findFirst().orElse(0);
  }
    ...
    return ResultPaginationDTO.builder()
        ...
        .data(reviews.getContent().stream().map(review -> {
          ...
          return ReviewProductDTO.builder().reviewId(review.getId()).firstName(firstName)
              ...
              .description(review.getDescription()).createdAt(review.getCreatedAt())
